...
parser.add_argument('--mlp_out', type=int, default=2, help='Number of MLP layer')
...
for epoch in range(1, args.epoch+1):
    ...
    if data.task == 'classification':
        acc_train = metric_fn(output[data.idx_train].max(1)[1], label[data.idx_train])
    if data.task == 'regression':
        acc_train = (loss_train - loss).sqrt()
    loss_train.backward()
    optimizer.step()
    ...
    if data.task == 'classification':
        acc_val = metric_fn(output[data.idx_val].max(1)[1], label[data.idx_val])
    if data.task == 'regression':
        acc_val = (loss_val - loss).sqrt()
...
if data.task == 'classification':
    acc_test = metric_fn(output[data.idx_test].max(1)[1], label[data.idx_test])
if data.task == 'regression':
    acc_test = loss_test.sqrt()
print('======================Testing======================')
